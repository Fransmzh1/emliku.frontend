...
const Registration = () => {
  ...
  const handleRequestPin = async (e) => {
    ...
    try {
      const response = await axios({
        method: 'post',
        url: config.BACKEND_URL + '/registration/requestpin',
        data: { email: email },
  ...
  const handleSubmitPin = async (e) => {
    e.preventDefault()
    if (!checkPin || !email) return
    try {
      const response = await axios({
        method: 'post',
        url: config.BACKEND_URL + '/registration/emailcheck',
        data: { email: email, password: checkPin },
        headers: { 'Content-Type': 'application/json' },
